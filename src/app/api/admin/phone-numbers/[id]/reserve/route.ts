...
// Validation schema for reserving phone numbers
const reservePhoneNumberSchema = z.object({
  reason: z.string().min(1, 'Reason is required').max(500, 'Reason must be 500 characters or less'),
  reservedUntil: z.union([z.string().datetime(), z.literal('')]).optional(),
...
export async function POST(
  ...
  try {
    ...
    const validatedData = reservePhoneNumberSchema.parse(body);
    ...
    // Check if phone number can be reserved (must be available)
    if (phoneNumber.status !== 'available') {
      return NextResponse.json(
        { error: `Cannot reserve phone number with status: ${phoneNumber.status}. Only available numbers can be reserved.` },
        { status: 400 }
      );
    }

    // Create admin note with reservation details
    const reservationNote = `RESERVED by ${user.name || user.email} on ${new Date().toISOString()}\nReason: ${validatedData.reason}${validatedData.reservedUntil && validatedData.reservedUntil !== '' ? `\nReserved until: ${validatedData.reservedUntil}` : '\nReserved for 7 days'}${validatedData.notes ? `\nNotes: ${validatedData.notes}` : ''}`;
    ...
    return NextResponse.json({
      message: 'Phone number reserved successfully',
    ...
    return NextResponse.json(
      { error: 'Failed to reserve phone number' },
      { status: 500 }
    );
  }
} 