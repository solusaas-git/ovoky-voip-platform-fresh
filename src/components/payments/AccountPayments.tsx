...
export function AccountPayments({ accountId }: AccountPaymentsProps) {
  ...
  const userInfoCache = new Map<string, { userName: string; userEmail: string; companyName?: string } | null>();

  // Helper function to get user information by email
  const getUserInfoByEmail = async (email: string): Promise<{ userName: string; userEmail: string; companyName?: string } | null> => {
    // Check cache first
    ...
    try {
      const response = await fetch(`/api/users?search=${encodeURIComponent(email)}&limit=1&include_onboarding=true`, {
      ...
      if (response.ok) {
        ...
        if (user) {
          const userInfo = {
            ...
            companyName: (user as any).onboarding?.companyName || undefined
          };
  ...
  const exportToCsv = () => {
    ...
    const csvContent = [
      ...
      ...paymentsData.payments.map(payment => [
        ...
        ...(isAdminViewingAll ? [
          payment._mongoData?.companyName || payment._mongoData?.userName || payment._mongoData?.userEmail || 'Unknown User',
          payment.i_account || ''
        ] : []),
        payment.amount || '',
        payment.currency || '',
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Sort users by company name from A to Z, with users without company names at the end
      const sortedUsers = usersWithSippyAccounts.sort((a: User, b: User) => {
        const companyA = a.onboarding?.companyName || '';
        const companyB = b.onboarding?.companyName || '';
        ...
        // If only A has company name, A comes first
        if (companyA && !companyB) {
          return -1;
        }
        
        // If only B has company name, B comes first
        if (!companyA && companyB) {
          return 1;
        }
        
        // If neither has company name, sort by email
                      ...
                      {user?.role === 'admin' && !accountId && (
                        <TableCell className="border-r border-muted/30 pt-4 pb-4 px-6">
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {payment._mongoData?.companyName ? 
                                (payment._mongoData?.userName || payment._mongoData?.userEmail || `Account: ${payment.i_account}`) :
