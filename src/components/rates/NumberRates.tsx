...
/*
 * NumberRates Component - Color Usage Philosophy:
 * 
...
import { UserNumberRates } from './UserNumberRates';

interface NumberRateDeck {
  id: string;
  name: string;
  description: string;
  currency: string;
...
interface NumberRateDecksData {
  rateDecks: NumberRateDeck[];
  total: number;
}

interface NumberRateDecksStats {
  total: number;
  active: number;
  totalRates: number;
...
export function NumberRates() {
  ...
  const [rateDecksData, setRateDecksData] = useState<NumberRateDecksData | null>(null);
  ...
  const [editingRateDeck, setEditingRateDeck] = useState<NumberRateDeck | null>(null);
  ...
  const [rateDeckToDelete, setRateDeckToDelete] = useState<NumberRateDeck | null>(null);

  // Assignment dialog state
  const [assignmentDialogOpen, setAssignmentDialogOpen] = useState(false);
  const [rateDeckToAssign, setRateDeckToAssign] = useState<NumberRateDeck | null>(null);

  // Calculate stats function
  const calculateStats = useCallback((rateDecks: NumberRateDeck[]): NumberRateDecksStats => {
    const stats: NumberRateDecksStats = {
  ...
  const fetchRateDecks = useCallback(async (page: number = currentPage, customRecordsPerPage?: number) => {
    try {
      ...
      const response = await fetch(`/api/rates/numbers/decks?${queryParams}`, {
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          const emptyData: NumberRateDecksData = {
            rateDecks: [],
            total: 0,
          };
        ...
        throw new Error(errorData.error || `HTTP ${response.status}: Failed to fetch number rate decks`);
      }

      const data: NumberRateDecksData = await response.json();
      console.log('Number rate decks API response:', { deckCount: data.rateDecks?.length });
      
      setRateDecksData(data);
      setTotalRecords(data.total || data.rateDecks?.length || 0);
      setLastRefresh(new Date());
    } catch (err) {
      console.error('Error fetching number rate decks:', err);
      
      let errorMessage = 'Failed to fetch number rate decks';
  ...
  const exportToCsv = () => {
    ...
    link.setAttribute('download', `number_rate_decks_${format(new Date(), 'yyyy-MM-dd')}.csv`);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingRateDeck 
        ? `/api/rates/numbers/decks/${editingRateDeck.id}`
        : '/api/rates/numbers/decks';
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(`API endpoint not implemented yet. Please implement ${method} ${url} to ${editingRateDeck ? 'update' : 'create'} rate decks.`);
        }
        
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        throw new Error(errorData.error || `Failed to ${editingRateDeck ? 'update' : 'create'} number rate deck`);
      }

      toast.success(`Number rate deck ${editingRateDeck ? 'updated' : 'created'} successfully`);
      ...
      console.error('Error saving number rate deck:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to save number rate deck';
      toast.error(errorMessage);
    }
  };

  const handleEdit = (rateDeck: NumberRateDeck) => {
  ...
  const handleDuplicate = async (rateDeck: NumberRateDeck) => {
    try {
      const response = await fetch(`/api/rates/numbers/decks/${rateDeck.id}/duplicate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        // If API doesn't exist yet (404), show helpful message
        if (response.status === 404) {
          throw new Error(`API endpoint not implemented yet. Please implement POST /api/rates/numbers/decks/{id}/duplicate to duplicate rate decks.`);
        }
        
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        throw new Error(errorData.error || 'Failed to duplicate number rate deck');
      }

      toast.success('Number rate deck duplicated successfully');
      fetchRateDecks();
    } catch (error) {
      console.error('Error duplicating number rate deck:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to duplicate number rate deck';
      toast.error(errorMessage);
    }
  };

  const handleDeleteClick = (rateDeck: NumberRateDeck) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/rates/numbers/decks/${rateDeckToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        // If API doesn't exist yet (404), show helpful message
        if (response.status === 404) {
          throw new Error(`API endpoint not implemented yet. Please implement DELETE /api/rates/numbers/decks/{id} to delete rate decks.`);
        }
        
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        throw new Error(errorData.error || 'Failed to delete number rate deck');
      }

      toast.success('Number rate deck deleted successfully');
      setDeleteDialogOpen(false);
      setRateDeckToDelete(null);
      fetchRateDecks();
    } catch (error) {
      console.error('Error deleting number rate deck:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to delete number rate deck';
      toast.error(errorMessage);
    }
  };

  const handleAssignUsers = (rateDeck: NumberRateDeck) => {
    setRateDeckToAssign(rateDeck);
    setAssignmentDialogOpen(true);
  };

  const handleManageRates = (rateDeck: NumberRateDeck) => {
    // Navigate to rate management for this deck
    window.location.href = `/rates/numbers/decks/${rateDeck.id}/rates`;
  };
      ...
      <div className="min-h-[400px] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{t('rates.numberRates.description')}</p>
          </div>
        </div>
      </div>
    );
        ...
        <CardHeader>
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <CardDescription>{t('rates.numberRates.errors.fetchFailed')}</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="space-y-6">
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleFormSubmit}>
                  <DialogHeader>
                    <DialogTitle>
                      {editingRateDeck ? t('rates.numberRates.form.title.edit') : t('rates.numberRates.form.title.add')}
                    </DialogTitle>
                    <DialogDescription>
                      {editingRateDeck ? t('rates.numberRates.description') : t('rates.numberRates.description')}
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="name" className="text-right">
                        {t('rates.numberRates.form.fields.name')}
                      </Label>
                      <Input
                        id="name"
                        value={formData.name}
                        ...
                        placeholder={t('rates.numberRates.form.fields.namePlaceholder')}
                        required
                      />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="description" className="text-right">
                        {t('rates.numberRates.form.fields.description')}
                      </Label>
                      <Textarea
                        id="description"
                        ...
                        className="col-span-3"
                        placeholder={t('rates.numberRates.form.fields.descriptionPlaceholder')}
                        rows={3}
                      />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="currency" className="text-right">
                        {t('rates.numberRates.form.fields.currency')}
                      </Label>
                      <Select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('rates.numberRates.stats.totalDecks')}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('rates.numberRates.stats.activeDecks')}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('rates.numberRates.stats.totalRates')}</p>
                <p className="text-2xl font-bold text-brand">
                  {stats.totalRates}
                </p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('rates.numberRates.stats.assignedUsers')}</p>
                <p className="text-2xl font-bold text-brand">
        ...
        <Card>
          <CardContent className="text-center py-12">
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-lg font-semibold" style={{ color: colors.primary }}>
                  {t('rates.numberRates.errors.noDecksFound')}
                </h3>
                <p className="text-muted-foreground">
                                      {isAdmin 
                      ? t('rates.numberRates.errors.noDecksFound')
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <Table>
                                  <TableHeader>
                    <TableRow>
                      <TableHead>{t('rates.numberRates.table.headers.name')}</TableHead>
                      <TableHead>{t('rates.numberRates.table.headers.description')}</TableHead>
                      <TableHead>{t('rates.numberRates.table.headers.currency')}</TableHead>
                      <TableHead className="text-center">{t('rates.numberRates.table.headers.rateCount')}</TableHead>
                      <TableHead className="text-center">{t('rates.numberRates.table.headers.assignedUsers')}</TableHead>
                      <TableHead className="text-center">{t('rates.numberRates.table.headers.status')}</TableHead>
                      <TableHead className="text-center">{t('rates.numberRates.table.headers.isDefault')}</TableHead>
                      <TableHead>{t('rates.numberRates.table.headers.created')}</TableHead>
                      <TableHead className="text-right">{t('rates.numberRates.table.headers.actions')}</TableHead>
                    </TableRow>
                  </TableHeader>
                <TableBody>
                  {rateDecksData?.rateDecks.map((deck) => (
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Delete Number Rate Deck</DialogTitle>
            <DialogDescription>
